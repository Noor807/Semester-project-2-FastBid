 ...
 *
 * @param {Array<Object>} auctions - List of auction post objects.
 * @param {string} [auctions[].id="unknown"] - Unique ID of the auction post.
 * @param {Object} [auctions[].media={}] - Media details for the auction post.
 * @param {string} [auctions[].media.url=""] - Image URL of the auction.
 * @param {string} [auctions[].media.alt="No description"] - Image alt text for the auction.
 * @param {string} [auctions[].title="Untitled Auction"] - Title of the auction.
 * @param {Object} [auctions[].seller={}] - Seller details.
 * @param {string} [auctions[].seller.name="Unknown Seller"] - Seller's name.
 * @param {string} [auctions[].created="Date"] - Creation date in ISO format.
 * @param {Array<string>} [auctions[].tags=[]] - Tags associated with the auction.
 */
export function renderAuctionPosts(auctions) {
  ...
  auctions.forEach((auction) => {
    ...
    img.alt = auction.media[0]?.alt || "No description";
...
/**
 * Determines if an auction has ended based on the end date.
 *
 * @param {string} endsAt - The ISO date string when the auction ends.
 * @returns {string} - Returns "Listing ended" if the auction has ended, otherwise an empty string.
 */
