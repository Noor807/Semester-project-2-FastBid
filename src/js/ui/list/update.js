...
export async function handleEditAuctionFormSubmit(e, auctionId) {
  ...
  if (!title && !description && !mediaUrl && !mediaAlt && selectedTags.length === 0) {
    alert("At least one of Title, Description, or Media is required.");
    return;
  }
  ...
  try {
    ...
    alert("Auction listing updated successfully!");
    ...
    alert("Failed to update auction listing: " + error.message);
  }
}
